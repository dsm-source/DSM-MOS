    ...
    <div className="p-6 space-y-6 max-w-5xl">
      ...
      <Tabs defaultValue="detail">
          ...
          {canWrite && !isTerminal && (
            <div className="rounded-xl border p-4 flex items-center gap-3 flex-wrap">
                ...
                onValueChange={async (v) => {
                  try {
                    await transition.mutateAsync({
                      id: data.id,
                      next: v as SalesOrderStatus,
                    });
                    toast.success(
                      `Status diubah ke ${STATUS_LABEL[v as SalesOrderStatus]}`,
                    );
                  } catch (e) {
                    notifyError(e);
                  }
              ...
              >
                ...
                <SelectContent>
                  {SALES_ORDER_STATUSES.filter((s) => s !== data.status).map(
                    (s) => (
        ...
        <TabsContent value="history">
          <StatusHistory salesOrderId={data.id} />
        </TabsContent>
      </Tabs>
    </div>
  );
