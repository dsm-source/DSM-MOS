...
- `user_roles`, `has_role()`, `claim_first_admin()`: bootstrap admin pertama sudah rapi (user pertama yang autentikasi bisa klaim admin kalau `user_roles` masih kosong, lalu admin itu yang provision user berikutnya manual). **Update 2026-08-15**: signup publik sudah dimatikan total (`auth.enable_signup = false` di `supabase/config.toml`, tab "Daftar" dihapus dari `auth.tsx`) supaya orang tak dikenal tidak bisa daftar sendiri. Konsekuensinya, prosedur bootstrap admin pertama berubah: user pertama **tidak lagi** dibuat lewat form signup, melainkan lewat Supabase Studio (Authentication → Add user) atau `supabase.auth.admin.createUser` langsung — baru setelah itu login lewat "Masuk" dan klik "Klaim admin pertama" di dashboard. Perlu dipastikan setting `enable_signup=false` ini juga diterapkan di project remote saat deploy (Task M0.1), karena `config.toml` lokal tidak otomatis sync ke remote tanpa langkah tambahan.
...
9a. **BARU (ditemukan saat verifikasi lokal, 2026-08-15)**: fungsi `has_role(uuid, app_role)` tidak pernah diberi `GRANT EXECUTE TO authenticated` (beda dengan `has_any_role()` yang sudah benar di migration `20260723061827`). Karena RLS policy `user_roles` (dan kemungkinan tabel lain) memanggil `has_role()` di `USING`/`WITH CHECK`, **setiap** user authenticated — bukan cuma non-admin — gagal dengan `permission denied for function has_role` begitu ada query ke tabel manapun yang memakainya, termasuk tepat setelah signup pertama kali. Sudah diperbaiki lewat migration baru `20260815000000_fix_has_role_grant.sql` (`GRANT EXECUTE ... TO authenticated, service_role`) dan diverifikasi hilang setelah apply ke local stack. **Migration ini perlu ikut di-deploy di Task M0.1**, dan patut dicek apakah fungsi lain (selain `has_any_role`) punya gap grant serupa sebelum deploy ke remote.
...
- [x] Task M0.2: Migration "fix-viewer-rls" — kembalikan akses SELECT role `viewer` di 9 tabel yang keliru di-exclude (temuan C.7): `deliveries`, `delivery_items`, `engineering_jobs`, `engineering_job_history`, `material_statuses`, `production_batches`, `production_batch_steps`, `qc_inspections`, `sales_order_status_history`. **Selesai** di `supabase/migrations/20260726000000_m0_foundation.sql` (bagian 2) — diterapkan dan diverifikasi jalan tanpa error di local stack. Belum di-deploy ke remote (Task M0.1).
- [x] Task M0.3: Buat migration baru: tabel `operators` (id, name, employee_number nullable, is_active, created_at, created_by) + RLS (SELECT semua peran, INSERT/UPDATE/DELETE production_planning+admin). **Selesai** di `supabase/migrations/20260726000000_m0_foundation.sql` (bagian 1). Belum di-deploy ke remote (Task M0.1). Catatan: migration yang sama juga sudah membawa fix `get_engineering_workload()` (temuan C.6 / Task M2.2 bagian backend) — bagian frontend `use-workload.ts` di M2.2 belum diverifikasi ulang.
- [x] Task M0.4: Halaman admin "Kelola User" — buat akun manual (email+password sementara) + assign role, tanpa self-signup (PRD §M8, dipindah lebih awal karena M0 butuh test user per role). Catatan: `claim_first_admin()` sudah ada untuk bootstrap admin pertama — user pertama tetap perlu sign-up/sign-in Supabase Auth biasa dulu sebelum klaim admin. **Selesai dan terverifikasi end-to-end** di local Supabase stack (`.env` lokal + `supabase start`): signup → klaim admin pertama → buat user baru dengan role `sales` via dialog → role ter-assign dan tampil di tabel. Catatan: ini baru verifikasi di **local stack** (bukan project remote `jtzwawtfymljfqfrplib`), jadi Task 0.1/M0.1 (deploy ke remote) masih outstanding terpisah.
