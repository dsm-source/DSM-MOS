...
### M7 — Delivery
UI: list rencana pengiriman + form catat rencana (bukan cetak dokumen) — diisi admin delivery dari kantor berdasarkan laporan driver. Gantt chart jadwal pengiriman (`gantt-task-react`), toggle Weekly/Monthly, warna per status, merah+label untuk yang lewat rencana.
...
| 10 | `estimated_delivery_date` TIDAK auto-fill ke `deliveries.planned_delivery_date` | Belum diperdalam | Tidak berubah — masih rekomendasi desain, boleh direvisit kalau user minta auto-fill. |
